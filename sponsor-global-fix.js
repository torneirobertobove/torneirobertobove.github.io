...
function logoCandidates(url){const d=domainOf(url);if(!d)return[];return[
  `https://${d}/logo.svg`,
  `https://${d}/logo.png`,
  `https://${d}/assets/logo.svg`,
  `https://${d}/assets/logo.png`,
  `https://${d}/images/logo.svg`,
  `https://${d}/images/logo.png`,
  `https://logo.clearbit.com/${d}`,
  `https://www.google.com/s2/favicons?domain=${d}&sz=128`
]}
function logoHtml(s){const c=logoCandidates(s.url);if(!c.length)return `<div class="sponsor-logo sponsor-logo-fallback">${esc(s.nome.slice(0,1).toUpperCase())}</div>`;const fallback=c[c.length-1];return `<div class="sponsor-logo-wrap"><img class="sponsor-logo" src="${esc(c[0])}" data-logo-candidates="${esc(JSON.stringify(c))}" data-logo-index="0" alt="Logo ${esc(s.nome)}" onerror="window.__sponsorLogoFallback&&window.__sponsorLogoFallback(this)"><span class="sponsor-logo-fallback">${esc(s.nome.slice(0,1).toUpperCase())}</span></div>`}
window.__sponsorLogoFallback=function(img){try{const list=JSON.parse(img.dataset.logoCandidates||'[]'),i=Number(img.dataset.logoIndex||0)+1;if(i<list.length){img.dataset.logoIndex=String(i);img.src=list[i]}else{img.style.display='none';img.parentElement?.querySelector('.sponsor-logo-fallback')?.classList.add('show')}}catch(e){img.style.display='none';img.parentElement?.querySelector('.sponsor-logo-fallback')?.classList.add('show')}};

/* CAROSELLO SPONSOR TABELLONE - solo presentazione */
function renderBoardGlobal(){
 ...
 #tabelloneSponsor .sponsor-item{flex:0 0 100%;min-height:145px;padding:18px 22px;border-radius:13px;display:flex;flex-direction:column;align-items:center;justify-content:center;box-sizing:border-box;background:rgba(255,255,255,.92);border:1px solid rgba(255,255,255,.95);color:#111827;text-decoration:none;box-shadow:0 8px 20px rgba(0,0,0,.2);transition:transform .2s,box-shadow .2s}
 #tabelloneSponsor .sponsor-item:hover{transform:translateY(-1px);box-shadow:0 10px 24px rgba(0,0,0,.25)}
 #tabelloneSponsor .sponsor-logo-wrap{height:64px;min-width:120px;display:flex;align-items:center;justify-content:center;margin-bottom:9px;position:relative}
 #tabelloneSponsor .sponsor-logo{display:block;max-width:210px;max-height:60px;width:auto;height:auto;object-fit:contain}
 #tabelloneSponsor .sponsor-logo-fallback{display:none;width:58px;height:58px;line-height:58px;text-align:center;border-radius:50%;font-weight:900;font-size:28px;background:#fff;border:1px solid #d1d5db;box-shadow:0 4px 12px rgba(0,0,0,.15)}
 #tabelloneSponsor .sponsor-logo-fallback.show{display:block}
 #tabelloneSponsor .sponsor-name{font-weight:900;font-size:21px;line-height:1.15;letter-spacing:.2px}
 ...
 @media(max-width:600px){#tabelloneSponsor{margin-top:22px;padding:15px 9px 10px;border-radius:15px}#tabelloneSponsor:before{font-size:8px;letter-spacing:1px}#tabelloneSponsor .sponsor-item{min-height:125px;padding:14px}#tabelloneSponsor .sponsor-logo{max-width:160px;max-height:48px}#tabelloneSponsor .sponsor-logo-wrap{height:52px}#tabelloneSponsor .sponsor-name{font-size:18px}#tabelloneSponsor .sponsor-desc{font-size:11px}}
 `;document.head.appendChild(style);
 async function load(){try{const sponsors=collect(await getTornei());if(!sponsors.length){track.innerHTML='<div class="sponsor-empty">Nessuno sponsor configurato</div>';return}track.innerHTML=sponsors.map(s=>{const inner=`${logoHtml(s)}<span class="sponsor-name">${esc(s.nome)}</span>${s.descrizione?`<span class="sponsor-desc">${esc(s.descrizione)}</span>`:''}`;return s.url?`<a class="sponsor-item" href="${esc(s.url)}" target="_blank" rel="noopener noreferrer">${inner}</a>`:`<div class="sponsor-item">${inner}</div>`}).join('');const items=[...track.querySelectorAll('.sponsor-item')];let index=0;const move=()=>{if(items.length<2){track.style.transform='translateX(0)';return}index=(index+1)%items.length;track.style.transform=`translateX(calc(-${index} * (100% + 14px)))`};if(items.length>1){setTimeout(move,2500);setInterval(move,5500)}}catch(e){console.error('SPONSOR GLOBAL TABELLONE:',e);track.innerHTML='<div class="sponsor-empty">Sponsor non disponibili</div>'}}
 load();return true;
}
