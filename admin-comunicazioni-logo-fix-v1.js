...
function findPhone(obj){const re=/(telefono|tel|cellulare|cell|mobile|phone|whatsapp|numero)/i,seen=new Set();function walk(v,k){if(v==null)return '';if(typeof v==='string'||typeof v==='number'){if(re.test(String(k||''))){const p=normalPhone(v);if(p.length>=8)return p}return ''}if(typeof v!=='object')return '';if(seen.has(v))return '';seen.add(v);for(const key of Object.keys(v)){const p=walk(v[key],key);if(p)return p}return ''}return walk(obj,'')}

function participantName(p){return p?.nome_giocatore||p?.nome||p?.nominativo||[p?.nome,p?.cognome].filter(Boolean).join(' ')||p?.email||'Partecipante'}
...
async function whatsappAuto(){const t=selectedTournament();if(!t){alert('Seleziona prima un torneo');return}const participants=await getParticipants(t);const msg=buildWhatsAppMessage(t);const root=document.getElementById('appContent');if(!root)return;

root.innerHTML=`<div class="page-head"><div><h1>WhatsApp</h1><p>${esc(t.nome)} · ${participants.length} partecipanti approvati</p></div><button class="btn" id="waBack">← Torna al torneo</button></div>

<div class="card feature-card">
<div class="card-head"><div><h2>Gruppo WhatsApp del torneo</h2><span class="notice">Partecipanti, messaggio e tabellone preparati automaticamente</span></div></div>

<div class="card-body">

<div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:18px">
<button type="button" class="btn primary" id="waCopyNumbers">📋 Copia tutti i numeri</button>
...
${participants.length
?participants.map((p,i)=>`<div class="list-item" style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">
<div style="flex:1"><strong>${i+1}. ${esc(p.name)}</strong><small style="display:block;margin-top:3px">+${esc(p.phone)}</small></div>
...
<strong>Come funziona:</strong> copia i numeri, apri WhatsApp, crea il gruppo e aggiungi i partecipanti. Poi invia il messaggio già preparato.
</div>
...
document.getElementById('waCopyNumbers')?.addEventListener('click',async()=>{const numbers=participants.map(p=>'+'+p.phone).join('\n');try{await copyText(numbers);alert('Numeri dei partecipanti copiati.')}catch(e){console.error(e);alert('Impossibile copiare i numeri.')}});

document.querySelectorAll('[data-wa-copy]').forEach(b=>b.addEventListener('click',async()=>{try{await copyText('+'+b.dataset.waCopy);b.textContent='✓ Copiato';setTimeout(()=>b.textContent='📋 Copia',1200)}catch(e){console.error(e)}}));

document.getElementById('waOpen')?.addEventListener('click',()=>{const text=document.getElementById('waText')?.value||msg;window.open('https://wa.me/?text='+encodeURIComponent(text),'_blank')});
}

const oldOpen=window.openAdminComPage;
window.openAdminComPage=p=>p==='whatsapp'?whatsappAuto():oldOpen?.(p);
