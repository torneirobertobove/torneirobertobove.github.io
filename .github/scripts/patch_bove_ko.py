...
if 'qCamp:Array(4).fill("")' not in s:
    ...
    fTopTime:[""],
    campioneTop:null,'''
    if old not in s: raise SystemExit('state defaults target not found')
    s=s.replace(old,new,1)

if 'qCamp: Array.isArray(state.qCamp)?state.qCamp:[]' not in s:
    old='            fTopRes: typeof state.fTopRes==="string"?state.fTopRes:"-",\n'
    new=old+'''            qCamp: Array.isArray(state.qCamp)?state.qCamp:[],
...
'''
    if old not in s: raise SystemExit('updateAndSync target not found')
    s=s.replace(old,new,1)
...
'''
    new=old+'''                if(Array.isArray(configurazione.qCamp)) state.qCamp=[...configurazione.qCamp];
...
'''
    if old not in s: raise SystemExit('loadState target not found')
    s=s.replace(old,new,1)
...
'''
    new=old+'''            if(!Array.isArray(state.qCamp)) state.qCamp=Array(4).fill("");
...
'''
    if old not in s: raise SystemExit('initDefaults target not found')
    s=s.replace(old,new,1)

if s.count('    state.fTopRes =\n        "-";') < 2: raise SystemExit('KO reset targets not found')
