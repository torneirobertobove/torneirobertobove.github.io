...
(function(){
  ...
  function playerName(g){
    return String(g?.nome_giocatore||[g?.nome,g?.cognome].filter(Boolean).join(' ')||g?.nome||'Giocatore').trim();
  }
  ...
  function buildPair(a,b){
    return {
      ...
      giocatore1:{
        ...
        nome:a.nome||'',
        cognome:a.cognome||'',
        nome_giocatore:a.nome_giocatore||'',
        email:a.email||''
      },
      giocatore2:{
        id:b.id,
        nome:b.nome||'',
        cognome:b.cognome||'',
        nome_giocatore:b.nome_giocatore||'',
        email:b.email||''
      },
  ...
  async function generaAccoppiamentiCasuali(){
    ...
    if(Array.isArray(t.coppie)&&t.coppie.length){
      const ok=confirm('Esistono già accoppiamenti. Vuoi sostituirli con un nuovo sorteggio casuale?');
      if(!ok)return;
    }
    ...
    t.coppie=pairs;
    t.configurazione.coppie=pairs;
    t.configurazione.accoppiamentoCasuale={
      generato:true,
      metodo:'casuale',
      generatoIl:new Date().toISOString(),
      partecipanti:shuffled.map(g=>g.id),
    ...
    if(client){
      const {error}=await client.from('tornei').update({configurazione:t.configurazione}).eq('id',t.id);
      if(error){
        console.error('Errore salvataggio accoppiamenti casuali:',error);
  ...
  function addButton(){
    ...
    button.style.cssText='white-space:nowrap;border-color:rgba(242,201,76,.35);'+(closed?'color:#f2c94c;background:rgba(242,201,76,.08);':'opacity:.45;cursor:not-allowed;');
  ...
  function init(){
    addButton();
    const box=document.getElementById('creaCoppieBox');
    if(box&&!observer){observer=new MutationObserver(schedule);observer.observe(box,{childList:true,subtree:true});}
