...
(function(){
  ...
  function currentTournament(){
    ...
    if(!state?.tornei) return null;
    return state.tornei.find(t=>String(t.id)===String(state.torneoSelezionato))||null;
  }

  function approvedPlayers(){
    return Array.isArray(window.iscrizioniTorneo) ? window.iscrizioniTorneo.filter(g=>g&&(g.stato==='approvato'||g.approvato===true)) : [];
  }
  ...
  function buildPair(a,b){
    return {id:Date.now()+Math.random(),giocatore1:{id:a.id,nome:a.nome||'',cognome:a.cognome||'',nome_giocatore:a.nome_giocatore||'',email:a.email||''},giocatore2:{id:b.id,nome:b.nome||'',cognome:b.cognome||'',nome_giocatore:b.nome_giocatore||'',email:b.email||''},generata_casualmente:true,metodo:'casuale'};
  }
  ...
  async function generaAccoppiamentiCasuali(){
    ...
    if(players.length<2){alert('Servono almeno 2 partecipanti approvati per creare le coppie.');return;}
    ...
    const shuffled=shuffle(players),pairs=[];
    for(let i=0;i+1<shuffled.length;i+=2)pairs.push(buildPair(shuffled[i],shuffled[i+1]));
    const bye=shuffled.length%2?shuffled[shuffled.length-1]:null;
    if(!t.configurazione)t.configurazione={};
    t.coppie=pairs;t.configurazione.coppie=pairs;
    t.configurazione.accoppiamentoCasuale={generato:true,metodo:'casuale',generatoIl:new Date().toISOString(),partecipanti:shuffled.map(g=>g.id),bye:bye?{id:bye.id,nome:playerName(bye)}:null};
    try{localStorage.setItem('padel_admin_state',JSON.stringify(window.adminState));}catch(e){console.error(e);}
    const client=getClient();
    if(client){const {error}=await client.from('tornei').update({configurazione:t.configurazione}).eq('id',t.id);if(error){console.error('Errore salvataggio accoppiamenti casuali:',error);alert('Le coppie sono state generate localmente, ma non è stato possibile salvarle su Supabase.');}}
    if(typeof window.renderCoppie==='function')window.renderCoppie();
    if(typeof window.syncDashboard==='function')window.syncDashboard();
    const byeText=bye?'\n\n⚠️ Partecipante rimasto senza coppia: '+playerName(bye):'';
    alert('Sorteggio completato!\n\nCoppie generate: '+pairs.length+byeText);
  }
  window.generaAccoppiamentiCasuali=generaAccoppiamentiCasuali;

  function addButton(){
    const box=document.getElementById('creaCoppieBox');if(!box)return;
    let wrap=document.getElementById('randomPairingTools');
    if(!wrap){
      wrap=document.createElement('div');wrap.id='randomPairingTools';wrap.setAttribute('data-admin-test','random-pairing');
    ...
    const t=currentTournament(),closed=isClosed(t),count=approvedPlayers().length;
    wrap.innerHTML='';
