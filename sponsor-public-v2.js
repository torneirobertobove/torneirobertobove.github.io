...
function addStyles() {
    if (document.getElementById('sponsor-public-v2-style')) {
        return;
    }
...
function sponsorAnimationStep(timestamp) {
    ...
    const visibleWidth = windowElement.clientWidth;
    ...
    const speed = 22;
    currentOffset -= (speed * elapsed) / 1000;
...
function startSponsorAnimation() {
    ...
    if (track.scrollWidth <= windowElement.clientWidth) {
        track.style.transform = 'translate3d(0,0,0)';
        return;
    }
...
function bindSponsorPause() {
    ...
    if (windowElement.dataset.pauseBound === 'true') {
        return;
    }

    windowElement.dataset.pauseBound = 'true';
...
function render(sponsors) {
    ...
    const cards = sponsors.map(function (sponsor) {
        const logo = sponsor.immagine
            ? '<img class="sponsor-public-logo"' +
              ' src="' +
              escapeHtml(sponsor.immagine) +
              '"' +
              ' alt="' +
              escapeHtml(sponsor.nome) +
              '"' +
              ' loading="lazy">'
            : '';

        const name =
            '<span class="sponsor-public-name">' +
            escapeHtml(sponsor.nome) +
            '</span>';
        ...
        if (sponsor.link) {
            return (
                '<a class="sponsor-public-item"' +
                ' href="' +
                escapeHtml(sponsor.link) +
                '"' +
                ' target="_blank"' +
    ...
    /*
     * Una sola card per ogni sponsor presente in Supabase.
     * NON viene duplicata la sequenza con cards + cards.
     */
...
async function loadSponsors() {
    ...
    try {
        ...
        const sponsors = (result.data || []).filter(
            function (sponsor) {
        ...
        console.error(
            'Errore caricamento sponsor pubblici:',
            error
        );
    }
}
